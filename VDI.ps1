 ...
 Param(
  #              [Parameter(Mandatory=$True,Position=1)]
   ...
   #             [Parameter(Mandatory=$True)]
                [string]$LogDir = "\\nam\wardfs\citrix\.GITHUB\Logs\VDI",
...
############ List Computers in Draining Mode  #########
Function DrainingUntilRestart
    {
        ...
        Write-Host "****************************************************"
            ...
            Foreach ($DeliveryController in $DeliveryControllers)
                {
                    write-host "Servers that are Draining Until Restart " $DeliveryController ":" -ForegroundColor Green
                    $Drains = Get-BrokerMachine -WindowsConnectionSetting DrainingUntilRestart  | select WindowsConnectionSetting,HostedMachineName,SummaryState
                    foreach ($Drain in $Drains)
                        {
                            if (Test-connection -ComputerName $drain.HostedMachineName -Count 1 -Quiet)
                            {
                                Try
                                    {      
                                        $LastBoot = (Get-WmiObject -Class Win32_OperatingSystem -computername $drain.HostedMachineName).LastBootUpTime
                                        $WMIsysuptime = (Get-Date) - [System.Management.ManagementDateTimeconverter]::ToDateTime($LastBoot)
                                        $WMIdays = $WMIsysuptime.Days
                                        $WMIDaystoHours = ($WMIsysuptime.Days)*24
                                        $WMIhours = $WMIsysuptime.hours
                                        $WMITotalHours = $WMIDaystoHours + $WMIhours                         
                                            if ($WMITotalHours -igt 48 -and $drain.SummaryState -like 'Available')
                                                {
 #                                                   New-BrokerHostingPowerAction -AdminAddress $DeliveryController -Action Reset -MachineName $drain.HostedMachineName | Out-Null
                                                    Write-Host $drain.HostedMachineName "has been restarted "                                                                               
                                                    if ($drain){$script:bad = '1'}
                                                }
                                            Elseif ($drain.SummaryState -like 'InUse')
                                                {
                                                    Write-Host $drain.HostedMachineName " (Users Logged in, Can't Restart)"
                                                    if ($drain){$script:bad = '1'}
                                                 }
                                            Elseif ($WMITotalHours -ilt 48)
                                                 {
                                                     Write-Host $drain.HostedMachineName " (Server has only been up for $WMITotalHours)"
                                                     if ($drain){$script:bad = '1'}
                                                 }
                                     } 
                                Catch
                        ...
                        foreach ($DG in $DGs)
                            {
                                ...
                                Write-Host MaintMode: $DG.InMaintenanceMode
                                Write-Host SessionsSupport: $DG.SessionSupport
                                Write-Host TotalDesktops: $DG.TotalDesktops
                                Write-Host Unregistered: $DG.DesktopsUnregistered
